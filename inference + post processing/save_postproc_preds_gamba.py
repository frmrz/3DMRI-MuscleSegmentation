...
output_dir_aiuto = "C:/Users/omobo/Desktop/no_zeros/GAMBA 0.8068/aiuto"
post_pred = Compose([
...
#------------------------------------------------------------------------------    
    ...
    post_proc_slice = rotate(outputs,90).astype(np.uint8)
    post_proc_slice = np.flipud(post_proc_slice)
    stackPIL = PIL.Image.fromarray(post_proc_slice)
    stackPIL.save(os.path.join(output_dir_aiuto,'{}.png'.format(image_id)))
    
    #cv2.imwrite(os.path.join(output_dir,'{}.png'.format(image_id)),post_proc_slice )
