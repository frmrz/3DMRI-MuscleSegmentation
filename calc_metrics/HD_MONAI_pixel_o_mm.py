# -*- coding: utf-8 -*-
"""
Created on Thu Jan 26 10:22:45 2023
...
#HD di monai è in mm o in pixels?
#confrontiamo la metrica di monai con quella di deep mind che sicuramente esprime la distanza di hausdorff in mm

#se entrambe danno lo stesso risultato allora anche quella di monai li da in mm

#test preso dalla repository : https://github.com/deepmind/surface-distance/blob/master/surface_distance_test.py
...
#sintax = surface_distance.compute_robust_hausdorff()
...
surface_distances = compute_surface_distances(mask_gt,
                              mask_pred,
                              spacing_mm)

hd_95_mm = compute_robust_hausdorff(surface_distances, 95)  #---> 8 mm
...
# pred_path = "C:/Users/omobo/Desktop/all image_2/COSCE/outputsTs-coscia/1_011_1_1_R_26.png"
# label_path = "C:/Users/omobo/Desktop/all image_2/COSCE/labelsTs-coscia/1_011_1_1_R_26.png"


pred_path = "C:/Users/omobo/Desktop/all image_2/COSCE/outputsTs-coscia/0_005_1_1_pred.nii"
label_path = "C:/Users/omobo/Desktop/all image_2/COSCE/labelsTs-coscia/0_005_1_1_mask.nii"


#-----------------------------------------------------------------------------------


test_files = [{"pred": pred_path, "label": label_path}]

transforms = Compose(
    [
        LoadImaged(keys=["pred", "label"]),
        EnsureChannelFirstd(keys=["pred", "label"]),
        Spacingd(keys=["pred", "label"],pixdim=(1,1,5),mode = ['nearest','nearest'])
        #Flipd(keys=["pred"], spatial_axis=2),
        #ToTensord(keys=["pred", "label"]),
        #DataStatsd(keys=["pred", "label"],prefix='Data', data_type=True, data_shape=True, value_range=True, data_value=True, additional_info=None, name='DataStats', allow_missing_keys=False)
...
# plt.subplot(1, 2, 1)
# plt.title('pred')
# plt.imshow(batch['pred'][0,0,:,:,15])

# plt.subplot(1, 2, 2)
# plt.title('label')
# plt.imshow(batch['label'][0,0,:,:,15])
# plt.show()
...
affine = batch['pred_meta_dict']['affine']

post_pred = Compose([AsDiscrete(to_onehot=13)])
...
hd95_monai = hd95_metric_batch.aggregate().cpu().numpy()

###############################################################################
#3D
import nibabel as nib

mask_gt_original = nib.load(label_path)
mask_gt_original = np.array(mask_gt_original.dataobj)

mask_pred_original = nib.load(pred_path)
mask_pred_original = np.array(mask_pred_original.dataobj)
#2D
# mask_gt_original = cv2.imread(label_path,cv2.IMREAD_GRAYSCALE)
# mask_pred_original = cv2.imread(pred_path,cv2.IMREAD_GRAYSCALE)

# plt.figure("check", (12, 6))
# plt.subplot(1, 2, 1)
# plt.title("gt")
# plt.imshow(mask_gt_original)

# plt.subplot(1, 2, 2)
# plt.title("pred")
# plt.imshow(mask_pred_original)
# plt.show()

mask_gt = np.zeros_like(mask_gt_original,bool)
mask_pred = np.zeros_like(mask_pred_original,bool)

mask_gt[mask_gt_original == 4] = 1
mask_pred[mask_pred_original == 4] = 1
...
hd_95_mm = compute_robust_hausdorff(surface_distances, 95)  
